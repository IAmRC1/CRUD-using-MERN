      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
            ...
            <h6 className="creator small">
              <Link 
                ...
                className={props.submittedBy === JSON.parse(localStorage.getItem("user")).username
                  ? "text-reset" : ""}
                style={{ pointerEvents: props.submittedBy === JSON.parse(localStorage.getItem("user")).username ? 'none' : 'auto' }}
              >
                {`@${props.submittedBy}`}
              </Link> 
              &#8728; 
          ...
          <div className="d-flex align-items-center justify-content-between">
            ...
            {!props.showEditAndDeleteBtn && props.type === "home" &&
              <a 
                role="button"
                id={props.id}
                className={`btn btn-heart ${props.type === 'home' && props.likes.includes(user_id) ? "btn-danger" : "btn-outline-danger"}`}
                data-tooltip={`${props.type === 'home' && props.likes.includes(user_id) ? "Unlike" : "Like"}`}
